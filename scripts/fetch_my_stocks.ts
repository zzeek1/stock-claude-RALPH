...
async function main() {
  ...
  for (const symbol of symbols) {
    ...
    // User asked to "fetch", so we overwrite to get latest.
    
    try {
      console.log(`Fetching ${symbol}...`);
      // Fetch daily candles, 1000 count (approx 4 years), forward adjusted
      // Note: SDK type definition might vary, using 'any' cast if needed or relying on inferred types
      // Period.Day is usually "day" string or enum.
